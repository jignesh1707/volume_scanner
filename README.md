...
Nifty / BankNifty ATM CE/PE options volume alert scanner. Polls **Fyers**
(API v3) using the daily access token shared across the trading-bot stack,
detects volume bursts, spikes, and consecutive-candle patterns, and pushes
Telegram alerts.
...
sudo -u trading nano /opt/volume_scanner/.env       # only FYERS_CLIENT_ID is needed (read-only)
...
> Fyers access tokens expire EOD. The token is **not** generated by this
> scanner — it's produced by `TickRenko/sync_fyers.bat` on the Windows
> machine (one login per morning for the whole bot stack) and scped to
> `/opt/shared/state/fyers_tokens.json`. `nifty_scanner` is already in
> that script's `SERVICES` list, so it restarts automatically when a new
> token lands.

For local debugging on the Windows machine, set `FYERS_TOKEN_PATH` in
`.env` to point at the locally-generated token, e.g.
`FYERS_TOKEN_PATH=C:\trading\TickRenko\state\fyers_tokens.json`.
